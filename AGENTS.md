...
│       ├── crawl/route.ts               ← POST — orchestrates Telegram+Yad2+manual → Claude extraction
...
>
> **Madlan was removed** (PerimeterX blocks plain fetch with HTTP 403 *and* headless/stealth browsers with a CAPTCHA — confirmed even from a residential IP; the listing pages are also a map-based SPA with no `__NEXT_DATA__`). Yad2 covers the same sale/rent inventory and works locally, so `/api/madlan`, the discovery source, and the settings card were all deleted. A headful `/api/scrape` mode (visible Chrome, persistent profile) *can* pass PerimeterX but needs a manual CAPTCHA solve per session — kept latent, not wired to any source.
...
(saves API costs) — except `yad2`/`telegram`, which are
...
# Used in /api/crawl to call /api/telegram, /api/yad2 internally
...
#
# Note: Apify was removed entirely (was a paid fallback). No paid scraping
# service is used. Yad2 is marked "local only" in the discovery UI — it works
# from a residential IP (local `npm run dev`) but is blocked by Cloudflare on
# cloud/datacenter IPs like Vercel's. (Madlan was removed entirely — PerimeterX.)
```
...
| Yad2 returns 0 posts on Vercel | Cloudflare blocks cloud/datacenter IPs | ⚠️ Known limit | No free workaround. Yad2 marked "local only" (works via `npm run dev` on a residential IP). Telegram MTProto is the free source that works on Vercel. |
| Madlan removed entirely | PerimeterX blocks plain fetch (403) AND headless/stealth browsers (CAPTCHA), even on residential IPs; listings are a map SPA with no `__NEXT_DATA__` | ❌ Removed | `/api/madlan`, discovery source, settings card deleted. Yad2 covers the same inventory. Headful `/api/scrape` can pass PerimeterX but needs manual CAPTCHA per session (kept latent). |
